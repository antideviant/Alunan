...
<div class="w3-padding" id="contact">
    <div class="w3-content w3-padding" style="max-width:600px">	
        ...
        $logged_in_user_id = $id_user; // Use the id_user of the logged-in user
        $SQL_list = "SELECT * FROM `post` JOIN `user` ON post.id_user = user.id_user WHERE post.id_user != $logged_in_user_id ORDER BY id_post DESC";
        $result = mysqli_query($con, $SQL_list);
		...
		<div class="w3-panel w3-border w3-border-brown w3-round-xxlarge">
			<div class="w3-row w3-small w3-padding-16 ">
				<div class="w3-col s3 w3-padding-16 w3-padding-small">
                    <a href="m-mprofile.php?id_post=<?PHP echo $id_post;?>&mid_user=<?PHP echo $mid_user;?>"><img src="upload/<?PHP echo $photo2;?>" class="w3-circle w3-image w3-border" ></a>
				</div>
